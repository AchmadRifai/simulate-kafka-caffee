	...
	public void ordering(KafkaMessage message) {
		...
		for (var item : items) {
			...
			if (menu.getStock() < qty) {
				producerService.sendToWaitres(String.format("Insuficient stock %s", menu.getName()), order.getEmail());
		...
		items.parallelStream()
				...
				.map(Optional::orElseThrow)
				.forEach(producerService::sendToHelper);
		producerService.sendToWaitres2("Pesanan anda akan datang", order.getEmail());
	}
